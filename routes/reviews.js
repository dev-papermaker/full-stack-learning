...
router.get(
  ...
  function(req, res) {
    // middleware.checkReviewExistence checks if a user already reviewed the memo, only one review per user is allowed
    Memo.findById(req.params.id, function(err, memo) {
      if (err) {
        req.flash("error", err.message);
        return res.redirect("back");
      }
      res.render("reviews/new", { memo });
    });
  }
);
    ...
    Memo.findById(req.params.id)
      ...
      .exec(function(err, memo) {
        ...
        Review.create(req.body.review, function(err, review) {
          ...
          //add author username/id and associated memo to the review
          review.author.id = req.user._id;
          review.author.username = req.user.username;
          review.memo = memo;
          //save review
          review.save();
          memo.reviews.push(review);
          // calculate the new average review for the memo
          memo.rating = calculateAverage(memo.reviews);
          //save memo
          memo.save();
